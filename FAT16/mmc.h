...
#define CS_HIGH()   SPI_TX_DONE; P5OUT |= 0x10  // Card Deselect
...
MMC_EXT         char mmc_buffer[512];
MMC_EXT		void initSPI (unsigned char u_UBR01 );


// mmc init

MMC_EXT		char initMMC (void);

// check if MMC card is present

MMC_EXT		char mmc_ping(void);

// send command to MMC

MMC_EXT		void mmcSendCmd (const char cmd, unsigned long data, const char crc);

// set MMC in Idle mode

MMC_EXT		char mmc_GoIdle();

// set MMC block length of count=2^n Byte

MMC_EXT		char mmcSetBlockLength (const unsigned long);

// read a size Byte big block beginning at the address.

MMC_EXT		char mmcReadBlock(const unsigned long address, const unsigned long count,volatile  unsigned char *pBuffer);
...
MMC_EXT		char mmcWriteBlock (const unsigned long address, const unsigned long count, unsigned char *pBuffer);
...
MMC_EXT		char mmcReadRegister(const char, const unsigned char, unsigned char *pBuffer);

// Read the Card Size from the CSD Register

MMC_EXT		unsigned long MMC_ReadCardSize(void);
MMC_EXT         void initSPI1 (unsigned char u_UBR01);
 /* _MMCLIB_H */
