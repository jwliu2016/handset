...
void initSPI (unsigned char u_UBR01)
{
  ...
  UBR01 = u_UBR01;                             // 0x02: UCLK/2 (4 MHz), works also with 3 and 4
  UBR11 = 0x00;                             // -"-
...
void initSPI1 (unsigned char u_UBR01)
{
...
char mmc_GoIdle()
{
  ...
  unsigned char i =mmcGetResponse();
 // PutChar(i);
  if(i!=0x01)
...
char mmcReadBlock(const unsigned long address, const unsigned long count, volatile unsigned char *pBuffer)
{
  unsigned long i = 0;
