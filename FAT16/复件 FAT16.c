...
void SendFileToUart(unsigned char *FileName)
{
  ...
  for (i = 0;i<m; i++)
  {
   ...
   if( ReadFile(FileName, SendBuf, i*512,512)==0xff)return;
    SendToUart1(SendBuf,512);
    
  }
  m=FileLength%512;
  ReadFile(FileName, SendBuf, FileLength-m,m)  ;
  SendToUart1(SendBuf,(unsigned int)m);
  
}



void RecvFileFromUart(void ){}
/*
{
   FileSize=FileSize-RxIndex;
   if((FileSize !=0)&&(RxIndex!=0))
   AppendData("Config  txt", RxBuf,(unsigned long )RxIndex);
   RxIndex = 0;
